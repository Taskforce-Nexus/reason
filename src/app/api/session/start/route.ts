...
export async function POST(req: NextRequest) {
  ...
  const sessionLimit = await checkPlanLimit(user.id, 'sessions_per_month')
  if (!sessionLimit.allowed) {
    return NextResponse.json({ error: 'plan_limit', message: sessionLimit.message, plan: sessionLimit.plan }, { status: 403 })
  }
